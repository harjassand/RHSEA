#!/usr/bin/env python
"""Phase 3: generator-only leak gate for forward/backward tasks."""
...
def main() -> int:
    ...
    ap.add_argument("--run_id", type=str, default="phase3")
    ap.add_argument("--auroc_max", type=float, default=0.55)
    ap.add_argument("--report", type=str, default="runs/phase3/leak_gate_phase3_report.json")
    ap.add_argument("--features_out", type=str, default="phase3_leak_features.json")
    ...
    if args.config:
        ...
        p3_cfg = cfg_data.get("phase3_gen", {})
        cfg = Phase3ChainConfig(**p3_cfg)
    else:
        cfg = Phase3ChainConfig()

    feats = []
    labels = []
    run_id = f"{args.run_id}_{args.task}_seed{args.seed}"
    for i in range(args.n):
        instance_id = f"i{i}"
        if args.task == "forward":
            inst = generate_phase3_forward(run_id, instance_id, cfg)
        else:
            inst = generate_phase3_backward(run_id, instance_id, cfg)
    ...
    features_out.write_text(json.dumps({"features": features_used, "task": args.task}, indent=2, sort_keys=True))

    if args.verbose:
        print("Phase3ChainConfig:", asdict(cfg))
        print("AUROC:", auroc)

    if auroc > args.auroc_max:
        print(f"FAIL leak gate ({args.task}): AUROC={auroc:.4f} > {args.auroc_max}")
        return 2
    print(f"PASS leak gate ({args.task}): AUROC={auroc:.4f} <= {args.auroc_max}")
